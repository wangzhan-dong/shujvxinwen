...
export default function TrueOwnershipBalance() {
  ...
  // 天平倾斜角度计算：分数越低，越倾向于“平台霸权(右倾)”，分数越高越“用户掌控(左倾)”
  // 分数范围 0 - 100。50分代表平衡。
  // 倾斜角度映射：0分映射为倾斜 20度（右重左高），100分映射为倾斜 -20度（左重右高）
  const score = selectedAsset.score;
  const tiltAngle = ((50 - score) / 50) * 20;

  return (
    <section id="ownership-balance" className="scroll-section py-24 bg-[#070714] text-white px-6 relative z-10 border-t border-white/10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black mb-4 bg-clip-text text-transparent bg-gradient-to-r from-sky-400 via-indigo-400 to-purple-500">
            你真的“拥有”你的虚拟资产吗？
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
          ...
          {/* Left Panel: Selector */}
          <div className="lg:col-span-4 flex flex-col justify-between bg-white/5 border border-white/10 p-6 rounded-[2.5rem] backdrop-blur-xl">
            <div>
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <span className="w-2 h-5 bg-sky-500 rounded-full"></span>
                选择你的虚拟资产
              </h3>
                ...
                {ASSETS_DATA.map((asset) => (
                  <button
                    ...
                    className={`w-full text-left px-5 py-4 rounded-2xl flex items-center justify-between transition-all border ${
                      selectedAsset.id === asset.id
                        ? "bg-gradient-to-r from-sky-500/20 to-purple-500/20 border-sky-500/50 text-white shadow-lg shadow-sky-500/5"
                  ...
                  >
                    ...
                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                      asset.score >= 60 ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                    }`}>
                      得分 {asset.score}
                    </span>
                  </button>
            ...
            <div className="mt-8 p-4 bg-sky-500/5 rounded-2xl border border-sky-500/10">
              <h4 className="text-xs font-bold text-sky-400 mb-1">⚖️ 司法冲突小贴士</h4>
          ...
          <div className="lg:col-span-4 flex flex-col justify-center items-center bg-black/20 border border-white/10 p-8 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden min-h-[350px]">
            <div className="absolute top-4 left-4 text-xs font-mono text-slate-500">
              PHYSICAL VS DIGITAL / 权属天平
            </div>

            {/* Scale SVG Container */}
            <div className="w-full max-w-[280px] h-[220px] flex items-end justify-center relative mt-6">
              
              {/* Stand / Base (Static) */}
              <svg className="absolute bottom-0 w-[120px] h-[100px] text-slate-600 z-0" viewBox="0 0 100 100" fill="currentColor">
                <path d="M45 10 h10 v80 h-10 z" />
                <path d="M20 90 h60 v10 h-60 z" />
                <circle cx="50" cy="15" r="8" className="text-slate-400" />
              </svg>

              {/* Lever Beam (Rotates based on score) */}
              <motion.g
              ...
              >
                {/* Lever Line */}
                <div className="absolute left-0 right-0 top-[14px] h-[3px] bg-slate-400 rounded-full"></div>
                ...
                <div className="absolute left-2 top-[15px] flex flex-col items-center origin-top" style={{ transform: `rotate(${-tiltAngle}deg)` }}>
                  {/* String */}
                  <svg width="40" height="60" viewBox="0 0 40 60" fill="none" className="text-slate-400">
                  ...
                  {/* Pan */}
                  <div className="w-14 h-6 bg-sky-500/20 border-2 border-sky-500 rounded-b-full shadow-lg shadow-sky-500/10 flex items-center justify-center text-[10px] font-bold text-sky-400">
                    玩家所有
                  </div>
                </div>
                ...
                <div className="absolute right-2 top-[15px] flex flex-col items-center origin-top" style={{ transform: `rotate(${-tiltAngle}deg)` }}>
                  ...
                  {/* Pan */}
                  <div className="w-14 h-6 bg-purple-500/20 border-2 border-purple-500 rounded-b-full shadow-lg shadow-purple-500/10 flex items-center justify-center text-[10px] font-bold text-purple-400">
                    平台条款
                  </div>
                </div>
              </motion.g>
            ...
            <div className="text-center mt-6 z-10">
              <div className="text-xs text-slate-500 uppercase font-mono">所有权掌控指数 (True Ownership Index)</div>
              <div className="text-4xl font-black mt-2 bg-gradient-to-r from-sky-400 to-purple-400 bg-clip-text text-transparent">
                {score}%
              </div>
              <div className="text-[10px] text-slate-400 mt-2 px-6">
                {score <= 30
          ...
          <div className="lg:col-span-4 bg-white/5 border border-white/10 p-6 rounded-[2.5rem] backdrop-blur-xl flex flex-col justify-between">
            <div>
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
              ...
              <div className="space-y-4">
                ...
                <div className="border-b border-white/5 pb-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-semibold text-slate-300">👪 遗赠继承权</span>
                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                      selectedAsset.specs.inheritable.status ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                    }`}>
                      {selectedAsset.specs.inheritable.text}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">{selectedAsset.specs.inheritable.desc}</p>
                </div>

                {/* Row 2: Tradable */}
                <div className="border-b border-white/5 pb-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-semibold text-slate-300">🔄 自由转让权</span>
                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                      selectedAsset.specs.tradable.status ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                    }`}>
                      {selectedAsset.specs.tradable.text}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">{selectedAsset.specs.tradable.desc}</p>
                </div>

                {/* Row 3: Offline */}
                <div className="border-b border-white/5 pb-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-semibold text-slate-300">🔌 离线可访问性</span>
                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                      selectedAsset.specs.offline.status ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                    }`}>
                      {selectedAsset.specs.offline.text}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">{selectedAsset.specs.offline.desc}</p>
                </div>

                {/* Row 4: Protection */}
                <div className="border-b border-white/5 pb-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-semibold text-slate-300">🛡️ 防单方封号/下架</span>
                    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-full ${
                      selectedAsset.specs.protection.status ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                    }`}>
                      {selectedAsset.specs.protection.text}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-relaxed">{selectedAsset.specs.protection.desc}</p>
                </div>
              </div>
            </div>
          </div>
